...
| `live_backend_reasoning_effort` | *(hidden, `low`)* | GPT-Live only: the backend's `reasoning.effort`. `none`/`minimal` answer fastest, `low` (default) is quick and still picks tools reliably, `medium`/`high` think longer, `default` leaves the field to OpenAI. |
| `live_backend_verbosity` | *(hidden, unset)* | GPT-Live only: the backend's `text.verbosity` (`low`, `medium`, `high`). Not sent when empty. Check acceptance with `tools/live_probe.py --verbosity low` before relying on it. |
| `live_builtin_web_search` | *(hidden, `true`)* | GPT-Live only (with `enable_web_search`): the backend model searches with OpenAI's built-in Responses `web_search` tool (no second API call, `web_search_model` unused). Set `false` to use the add-on's own `web_search` function as on gpt-realtime. If the API rejects the built-in tool the add-on falls back once by itself. |
